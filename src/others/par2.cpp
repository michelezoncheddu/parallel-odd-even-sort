...
template <typename T>
void f(unsigned thid, T *v, size_t end, int alignment, std::vector<std::unique_ptr<barrier>> &barriers, std::vector<unsigned> &swaps) {
    auto iter = 0;
    auto pos = thid * padding;
    while (!finished) {
        swaps[pos] = odd_even_sort(v, alignment, end);
        barriers[iter++]->wait();
        alignment = !alignment;
    }
}
...
int main(int argc, char const *argv[]) {
    ...
    for (unsigned i = 0; i < nw - 1; ++i) {
        ...
        threads[i] = std::make_unique<std::thread>(
                f<vec_type>, i, ptr + offset, chunk_len + (remaining > 0), offset % 2, std::ref(barriers), std::ref(swaps));
        offset += chunk_len + (remaining > 0);
        --remaining;
    }
    //std::cout << offset << " 2, " << chunk_len - 1 << std::endl;
    threads[nw - 1] = std::make_unique<std::thread>(
            f<vec_type>, nw - 1, ptr + offset, chunk_len - 1, offset % 2, std::ref(barriers), std::ref(swaps));
